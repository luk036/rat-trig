...
TEST_CASE("Testing spread function") {
    SUBCASE("Test with integer vectors") {
        ...
        CHECK(rat_trig::spread(v1, v2) == doctest::Approx(expected).epsilon(1e-10));
    }
    ...
    SUBCASE("Test with float vectors") {
        ...
        CHECK(rat_trig::spread(v1, v2) == doctest::Approx(expected).epsilon(1e-6f));
    }
